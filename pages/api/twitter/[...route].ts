...
export default async function handler(
  ...
  switch (req.query.route?.[0]) {
    case "details": {
      ...
      search.set(
        "user.fields",
        "id,description,public_metrics,location,username,url,name,created_at,pinned_tweet_id,entities"
      )
      search.set("expansions", "pinned_tweet_id")
      search.set("tweet.fields", "context_annotations,entities,geo")
      return fetch(
        `https://api.twitter.com/2/users/by/username/${name}?${search}`,
        {
          headers: {
    ...
    case "following": {
      ...
      return fetch(
        `https://api.twitter.com/2/users/${twitterId}/following?${search}`,
        {
          headers: {
            authorization: `bearer ${process.env.TWITTER_BEARER}`,
          },
